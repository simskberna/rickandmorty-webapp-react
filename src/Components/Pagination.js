...
const Pagination = ({ totalPosts, POST_PER_PAGE, paginate }) => {
  ...
  const paginateToStart= () => {
    if (prev < postNumbers.length && prev > 0) {
      paginate(prev)
    } else {
      paginate(postNumbers[0])
    }
  }
  const paginateToEnd = () => {
    if (next <= postNumbers.length && next > 1) {
      paginate(next)
    }
    else {
      paginate(postNumbers.length)
    }
    ...
    <ul className="inline-flex items-center -space-x-px">
        ...
        {postNumbers.map((number) => ( 
          ...
          <li onClick={() => { 
            ...
            setNext(number+1)
            setPrev(number-1)
        }}  key={number} className="cursor-pointer page-item z-10 px-3 py-2 leading-tight text-blue-600 border border-blue-300 bg-blue-50
